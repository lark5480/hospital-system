...
- 等保 / 七权分隔 / 审计作为**设计深度**体现在架构里,体现你理解医疗系统的特殊要求。
...
### 13.4 简历关键词(供提炼)
Spring Boot 3 + Java 21 · ArchUnit 架构守护 · DDD 限界上下文 · CQRS-lite · HL7 v2 / FHIR R4 防腐层 · 等保三级合规设计(审计 / 七权分隔 / EMPI) · 事件驱动(RabbitMQ) · API Gateway · Docker Compose / k3s 私有化 · GitHub Actions CI · 高单测覆盖。
